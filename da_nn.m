function Li = da_nn(z)
    ...
    num = size(z,2);
    Li = zeros(1,num);
    % Associate each observation
    for i = 1:num
        Dist_min = flintmax;
        nearest = 0;
        % For each observation, go through all landmarks to find the
        % nearest one
        for j = 1:State.Ekf.nL
            [Sigma, zhat] = get_SigmaZhat(j);
            Dist_ij = mahalanobis2(z(:,i), zhat, Sigma);%mahalanobis distance calculation
            % Update nearest distance value
            if Dist_ij < Dist_min 
                nearest = j;
                Dist_min = Dist_ij;
            end
        end
        % Condition to determine whether it should be associated
